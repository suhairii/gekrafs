...
<body>
	...
	<div class="container px-5">
		<h2 class="text-center mb-3">Edit Banner</h2>
		<?php
		include '../config/koneksi.php';
		$id = $_GET['id_banner'];
		$data = mysqli_query($koneksi, "select * from banner where id_banner='$id'");
		while ($d = mysqli_fetch_array($data)) {
		?>
			<form action="update_banner.php" method="POST" enctype="multipart/form-data">
				<div class="mb-3">
					<input type="hidden" name="id_banner" value="<?php echo $d['id_banner']; ?>">
					<label for="exampleInputEmail1" class="form-label">Judul</label>
					<input type="text" value="<?php echo $d['judul']; ?>" name="judul" class="form-control bg-light">
				</div>
				<div class="mb-3">
					<label for="exampleInputEmail1" class="form-label">Hashtag</label>
					<input type="text" value="<?php echo $d['hashtag']; ?>" name="hashtag" class="form-control bg-light">
				</div>
				<div class="mb-3">
